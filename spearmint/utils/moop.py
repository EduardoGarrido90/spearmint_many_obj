...
class MOOP(base):
    ...
    def __init__(self, objective_tasks, models, input_space, avg_over_hypers = True, constraint_means=[], extended_domination_rule = False):
	...
	self.alg = algorithm.nsga_II(gen = 1)
        ...
        #Only for BMOO algorithm.
        self.constraint_means = constraint_means
        self.extended_domination_rule = extended_domination_rule
    ...
    def apply_extended_domination_rule_to_values(self, values):
        new_values = np.zeros((len(values),self.input_space.num_dims+self.constraint_means.shape[1]))
        val_index=0
        for value in values:
            if(np.any(self.constraint_means[val_index]>0)):
                objective_values = np.empty(self.input_space.num_dims)
                objective_values[:] = sys.maxint
                constraint_values = np.array([x if x > 0 else 0 for x in self.constraint_means[val_index]])
                new_values[val_index] = np.append(objective_values, constraint_values)
            else:
                objective_values = value
                constraint_values = np.zeros(self.constraint_means.shape[1])
                new_values[val_index] = np.append(objective_values, constraint_values)
            val_index+=1
        return new_values
    ...
    # This method solves the problem using a grid
    def solve_using_grid_returning_values(self, grid = None):
	...
	return {'frontier':frontier, 'pareto_set':pareto_set}
...
class MOOP_basis_functions(base):
	...
	def __init__(self, funs, num_dims, constraints=[]):
	...
	def compute_pareto_front_and_set(self):
		...
		result = self.pop.compute_pareto_fronts()

		# If there is no solution we return None

		if len(result) == 0:
			return { 'pareto_set': None, 'frontier': None }
